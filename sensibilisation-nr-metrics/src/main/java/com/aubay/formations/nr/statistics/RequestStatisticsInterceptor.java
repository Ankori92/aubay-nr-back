...
public class RequestStatisticsInterceptor implements AsyncHandlerInterceptor {
	...
	public void afterCompletion(final HttpServletRequest request, final HttpServletResponse response,
		...
		// Get metrics
		final String path = getRequestMappingPath(handler, request);
		final int duration = getDuration();
		final int queries = getQueriesCount();
		final int length = getResponseLength(response);
	...
	public int getResponseLength(final HttpServletResponse response) {
		...
		if (response instanceof ContentCachingResponseWrapper) {
			final ContentCachingResponseWrapper cachedResponse = (ContentCachingResponseWrapper) response;
	...
	public String getRequestMappingPath(final Object handler, final HttpServletRequest request) {
		...
		if (handler instanceof HandlerMethod) {
			...
			path = method + " " + String.join(", ", handlerMethod.getMethodAnnotation(RequestMapping.class).path());
		}
		return path;
	}
	...
	public void postTreatments() {
		...
		if (BUFFER.size() >= 10000) {
			...
			LOG.warn("WARNING : Too many metrics in buffer");
			LOG.warn("Buffer have been cleared to prevent memory leaks");
			LOG.warn("Please verify metrics saving");
			LOG.warn("=================================");
		}
	}
